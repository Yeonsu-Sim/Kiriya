...
async function selectPlan(plan) {
  if (routeViewStore.routeViewPlan && routeViewStore.routeView?.length > 0 && !confirm('작업중인 경로가 사라집니다. 정말 새로 불러오시겠습니까?')) {
    return
  }
  selectedPlan.value = { ...plan} 
  
  // Assuming fetchRouteView fetches the full plan details including its route geometry
  routeViewStore.fetchRouteView(plan.pno) 
  
    routeViewStore.importRouteView(plan) // Import the full plan data
    routeViewStore.routeViewImportDialogShow = false
    routeViewStore.importRouteViewShow = true // Show the imported route view/controls
    routeViewStore.newRouteViewShow = false   // Hide new route creation view/controls
    routeViewStore.routeViewTrip = selectedTrip.value ? { ...selectedTrip.value } : null // Associate the trip context
}
