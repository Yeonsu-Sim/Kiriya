<template>
  <v-row justify="center" align="center" class="pa-4">
    <v-card color="surface" class="w-100 pa-4" style="max-width: 1000px">
      <h2 class="text-primary text-center ma-4">공지 작성</h2>
...
<script setup>
  ...
  const submit = async () => {
    ...
    if (valid) {
      ...
      try {
        const response = await axios.post('http://localhost:8080/api/notices', data)
        console.log('등록 성공:', response.data)

        successMessage.value = '공지사항이 등록되었습니다.'
        success.value = true
        ...
        errorMessage.value = '공지사항 등록에 실패했습니다.'
        error.value = true
      } finally {
        loading.value = false
      }
