<template>
  <v-row justify="center" align="center" class="pa-4">
    <v-card color="surface" class="w-100 pa-4" style="max-width: 1000px">
      <h2 class="text-primary text-center ma-4">공지 수정</h2>
      ...
      <v-card-text>
        <v-form ref="form" lazy-validation>
          ...
          <div class="d-flex ga-4">
            ...
            <v-btn color="secondary" :disabled="loading" variant="tonal" @click="cancel">
              취소
            </v-btn>
            <v-btn color="secondary" :disabled="loading" variant="outlined" @click.prevent.stop="showList">
              목록으로
            </v-btn>
          </div>
        </v-form>
...
<script setup>
  ...
  const submit = async () => {
    ...
    if (valid) {
      ...
      try {
        const id = route.params.id
        const response = await axios.put(`http://localhost:8080/api/notices/${id}`, data)
        ...
        errorMessage.value = '공지사항 수정에 실패했습니다.'
        error.value = true
      } finally {
        loading.value = false
      }
