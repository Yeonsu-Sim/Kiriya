<template>
  <AppTable :headers="headers" :items="items" domain="/support/notices"
		@search="onSearch($event)"
	>
		<template #write>
			<v-btn color="primary" @click="$router.push({ name: 'NoticeWrite' })">
				글쓰기
			</v-btn>
		</template>
...
<script setup>
	...
	const onSearch = async (query) => {
		...
		try {
			const res = await axios.get(`http://localhost:8080/api/notices?text=${query}`)
			...
			router.push({ tab: route.query.tab, query: { text: query } })
		} catch (e) {
